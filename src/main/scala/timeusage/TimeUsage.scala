...
object TimeUsage extends TimeUsageInterface:
  ...
  def timeUsageByLifePeriod(): Unit =
    ...
    finalDf.show()
    val typedSummary = timeUsageSummaryTyped(summaryDf)
    val typedFinal = timeUsageGroupedTyped(typedSummary)
    typedFinal.show()
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) =
    ...
    val primaryNeedsColumns = columnNames.filter(name => primaryNeeds.exists(name.startsWith)).map(col)
    val workingActivitiesColumns = columnNames.filter(name => workingActivities.exists(name.startsWith)).map(col)
    val otherActivitiesColumns = columnNames
      .filter(name => otherActivities.exists(name.startsWith))
      .filter(name => !name.startsWith("t1801") && !name.startsWith("t1805") && !name.startsWith("t1803"))
      .map(col)
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = primaryNeedsColumns.reduce((acc, x) => acc + x / 60).as("primaryNeeds")
    val workProjection: Column = workColumns.reduce((acc, x) => acc + x / 60).as("work")
    val otherProjection: Column = otherColumns.reduce((acc, x) => acc + x / 60).as("other")
    df
